 ...
 *
 * A `final class` and not a `final readonly class`: a readonly class is PHP 8.2 and this
 * branch supports PHP 8.1. The properties are readonly individually, which 8.1 does have.
 */
final class PageCategorySummaryRenderer
{
    ...
    public function renderForPageOfType(ServerRequestInterface $request, int $doktype, string $group): string
    {
        ...
        // is not the place to tell them it exists. Neither supported version declares a return
        // type for it - only an `@return array|false` docblock - so the result is checked with
        // `is_array()` rather than against `false`, which is also what the `main` implementation
        // does, where v14 declares the type and v13 does not.
